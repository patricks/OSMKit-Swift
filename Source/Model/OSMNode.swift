...
public class OSMNode: OSMElement {
    ...
    override init(xmlAttributes: [String: String]) {
        ...
        guard let latString = xmlAttributes[XMLAttributes.latitude.rawValue] else { return }
        guard let lonString = xmlAttributes[XMLAttributes.longitude.rawValue] else { return }

        guard let lat = Double(latString) else { return }
        guard let lon = Double(lonString) else { return }
